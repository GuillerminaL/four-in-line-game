class Timer {
    ...
    constructor(root) {
        ...
        this.interval = null;
        this.remainingSeconds = this.defaultTime*60;   //5 min default set
        this.setEvents();
    }
    ...
    showTimerResetInput() {
        let input = `<div class="time-input-box">
                        <div>
                            <label for="input">Minutes to play: </label>
                            <input id="minutes-input" type="number" name="minutes" min=1 max =60 value="" step="1">
                        </div>
                        <div>
    ...
    setInputBtnsEvents() {
        ...
        doneBtn.addEventListener("click", () => {
            let input = document.querySelector("#minutes-input").value;
            if ( input ) {
                let inputMinutes = (Number.isInteger(input)) ? input : Math.round(input);
                if( inputMinutes > 0 && inputMinutes < 60 ) {
                    this.stop();
                    this.remainingSeconds = inputMinutes * 60;
                    this.updateInterfaceTime();
                    MSG_BOX.innerHTML = "Press the start button to init the countdown";
